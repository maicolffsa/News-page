...
$firstcatpostsmall = DB::table('posts')->where('category_id',$firstcategory->id)->where('bigthumbnail',NULL)->limit(10)->get();
@endphp

    <div class="col-md-6 col-sm-6">
							<div class="bg-one">
	 <div class="cetagory-title"><a href="#">
		{{ $firstcategory->category_es }}
	 	 <span>
		Mas
...
{!! $livetv->embed_code  !!}<iframe
   src="http://live.piuraplustv.com/embed/video"
  title="PiuraPlusTV Live"
...
<div class="row">
	...
	@php
$fiftycategory = DB::table('categories')->skip(4)->first();

$fiftycatpostbig = DB::table('posts')->where('category_id',$fiftycategory->id)->where('bigthumbnail',1)->first();

$fiftycatpostsmall = DB::table('posts')->where('category_id',$fiftycategory->id)->where('bigthumbnail',NULL)->limit(3)->get();
@endphp
...
<div class="col-md-6 col-sm-6">
	<div class="bg-one">
		...
		<div class="row">
			<div class="col-md-6 col-sm-6">
				<div class="top-news">
					<a href="#"><img src="{{ asset($fiftycatpostbig->image) }}" alt="Notebook"></a>
					<h4 class="heading-02"><a href="#">
						@if(session()->get('lang')== 'spanish')
		{{ $fiftycatpostbig->title_es }}
		@else
		{{ $fiftycatpostbig->title_es }}
		@endif</a> </h4>
				</div>
			...
			<div class="col-md-6 col-sm-6">
				@foreach($fiftycatpostsmall as $row)
				<div class="image-title">
					<a href="#"><img src="{{ asset($row->image) }}" alt="Notebook"></a>
					<h4 class="heading-03"><a href="#">
		@if(session()->get('lang')== 'spanish')
...
$sixcatpostbig = DB::table('posts')->where('category_id',$sixcategory->id)->where('bigthumbnail',1)->first();

$sixcatpostsmall = DB::table('posts')->where('category_id',$sixcategory->id)->where('bigthumbnail',NULL)->limit(3)->get();
@endphp
...
<div class="col-md-9 col-sm-9">
	<div class="cetagory-title-02"><a href="#">Feature  <i class="fa fa-angle-right" aria-hidden="true"></i> all provincia news tab here <span><i class="fa fa-plus" aria-hidden="true"></i> All News  </span></a></div>
...
@php
$dis = DB::table('provincias')->get();
@endphp		
		<div class="cetagory-title-02"><a href="#">Search By Provincia<i class="fa fa-angle-right" aria-hidden="true"></i> <span><i class="fa fa-plus" aria-hidden="true"></i>   </span></a></div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
		<form action="{{ route('searchby.provincias') }}" method="get">
			@csrf
			<div class="row">
				<div class="col-lg-4">
 <select class="form-control" id="exampleSelectGender" name="provincia_id">
   <option disabled="" selected="">--Select Provincia--</option>
                @foreach($dis as $row)
  <option value="{{ $row->id }}">{{ $row->provincia_es}} </option>
                @endforeach
                        </select>
...
<script type="text/javascript">
   $(document).ready(function() {
         $('select[name="provincia_id"]').on('change', function(){
             ...
             if(provincia_id) {
                 $.ajax({
                     url: "{{  url('/get/distrito/frontend') }}/"+provincia_id,
                     type:"GET",
                     dataType:"json",
